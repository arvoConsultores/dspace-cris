...
/**
 * Service implementation class for the DSpaceObject. All DSpaceObject service
 * classes should extend this class since it implements some basic methods which
 * all DSpaceObjects are required to have.
 *
...
public abstract class DSpaceObjectServiceImpl<T extends DSpaceObject> implements DSpaceObjectService<T> {
		...
		for (int i = 0; i < values.size(); i++) {
			...
			if (authorityControlled) {
				...
				if (authorityRequired
					...
					throw new IllegalArgumentException("The metadata field \"" + metadataField.toString()
							+ "\" requires an authority key but none was provided. Vaue=\"" + values.get(i) + "\"");
				}
			}
			if (values.get(i) != null) {
				...
				for (int charPos = 0; charPos < dcvalue.length; charPos++) {
					if (Character.isISOControl(dcvalue[charPos]) && !String.valueOf(dcvalue[charPos]).equals("\u0009")
			...
			// An update here isn't needed, this is persited upon the merge of
			// the owning object
	...
	/**
	 * Protected method that deletes all metadata values from the database,
	 * should only be called when deleting the item.
	 * 
	 * @param context
	 *            the dspaceObject
	 * @param dso
	 *            the dspaceObject who's metadata we are to delete
	 */
	 ...
	 * The returned array will always have length >= 4
	 * <p/>
	 * When @param fill is true, elements that would be empty or null are
	 * replaced by Item.ANY
	 */
