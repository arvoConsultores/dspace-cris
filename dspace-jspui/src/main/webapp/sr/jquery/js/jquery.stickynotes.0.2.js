(function(jQuery) {
	...
	jQuery.fn.stickyNotes.prepareContainer = function(container) {
		...
		if (jQuery.fn.stickyNotes.options.controls) {
			jQuery("#sticky-container").html('<button id="add_note">Nuova nota</button>');			
	...
	jQuery.fn.stickyNotes.createNote = function() {
		...
		var _div_delete = 	JQ(document.createElement('div'))
							.addClass('jSticky-delete')
							.click(function(){jQuery.fn.stickyNotes.deleteNote(this);});
		var _div_wrap 	= 	JQ(document.createElement('div'))
		...
		JQ('#sticky-container').append(_div_wrap);
	...
	jQuery.fn.stickyNotes.stopEditing = function(note_id) {
		...
		note.text = JQ("#note-" + note_id).find('textarea').val();
		var _p_note_text = 	JQ(document.createElement('p')).attr("id", "p-note-" + note_id)
							.html(note.text);
		JQ("#note-" + note_id).find('textarea').replaceWith(_p_note_text); 
		JQ("#p-note-" + note_id).dblclick(function() {
	...
	jQuery.fn.stickyNotes.editNote = function(paragraph) {
		...
		var textarea = 	JQ(document.createElement('textarea')).attr("id", "textarea-note-" + note_id)
							.val(
									JQ("#note-" + note_id)
									.find('p')
									.html()
		...
		JQ("#p-note-" + note_id).replaceWith(textarea);
	...
	jQuery.fn.stickyNotes.renderNote = function(note) {
		var _p_note_text = 	JQ(document.createElement('p')).attr("id", "p-note-" + note.id)
							.html( note.text);		
		var _div_note 	= 	jQuery(document.createElement('div')).addClass('jStickyNote');
        var _div_background = jQuery.fn.stickyNotes.createNoteBackground();
		...
		JQ(_p_note_text).dblclick(function() {
			jQuery.fn.stickyNotes.editNote(this);
		});		
	}
    ...
    jQuery.fn.stickyNotes.createNoteBackground = function() {
        ...
        if (jQuery.browser.msie && jQuery.browser.version <= 6)  {
            background = JQ(document.createElement('div')).addClass("background").html('<img src="/sr/jquery/images/stickynotes/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'images/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
        } else {
            background = JQ(document.createElement('div')).addClass("background").html('<img src="/sr/jquery/images/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
        }
        return background;
