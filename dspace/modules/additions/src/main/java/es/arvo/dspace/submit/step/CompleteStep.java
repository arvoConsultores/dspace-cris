     ...
     // ARVO: si hay token y estamos en el item correspondiente te deja subir archivos. 
        ...
        if(revisionToken!=null){
        	...
        	if(revisionToken.getTipo().equalsIgnoreCase("R")){
        	    subInfo.getSubmissionItem().getItem().addMetadata("dc", "title", null,Item.ANY,getFirstMetadataValue(itemRevisado,"dc.title")+" (Revisión de)");
        	    ...
        	    itemRevisado.addMetadata("dc", "relation", "isrelatedtoRevision",Item.ANY, ConfigurationManager.getProperty("dspace.url")+"/handle/"+ subInfo.getSubmissionItem().getItem().getHandle());
        	    ...
        	    subInfo.getSubmissionItem().getItem().addMetadata("dc", "title", null,Item.ANY,getFirstMetadataValue(itemRevisado,"dc.title")+" (Juicio de)");
