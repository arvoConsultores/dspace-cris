 ...
 *
 */
public class SolrBrowseDAOConPrivados implements BrowseDAO
{
    public SolrBrowseDAOConPrivados(Context context)
    {
    	this.context = context;

    	boolean isSystemAdmin;
    	try {
    	    isSystemAdmin = AuthorizeManager.isAdmin(context);
    	    Group[] groups;
    	    ...
    	    log.error("Error configurando las comuniadades privadas",e);
    	    e.printStackTrace();
    	}  
    }
    ...
    /** Log4j log */
    private static Logger log = Logger.getLogger(SolrBrowseDAOConPrivados.class);
    ...
    // handle de la comunidad privada
    
    // el usuario actual puede ver los items de todas las colecciones incluida la privada
    private boolean allowedAllCollections=false;
    ...
    private void addStatusFilter(DiscoverQuery query)
    {
     ...
     // Adan: añadimos un filtro si el usuario no es de un grupo con permisos sobre comunidades y colecciones privadas
        		...
        		for(int i=0;i<privs.length;i++){
        			try {
        				...
        				if(dso!=null && dso instanceof Community){
        					query.addFilterQueries("NOT(location.comm:"+dso.getID()+")");    
        				}else if(dso!=null && dso instanceof org.dspace.content.Collection){
        					query.addFilterQueries("NOT(location.coll:"+dso.getID()+")");
        				}
            ...
            boolean isAscending) throws BrowseException
    {
        ...
        Comparator comparator = new SolrBrowseDAOConPrivados.FacetValueComparator();
