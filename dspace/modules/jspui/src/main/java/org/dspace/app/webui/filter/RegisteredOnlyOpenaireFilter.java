 ...
 * @version $Revision$
 */
public class RegisteredOnlyOpenaireFilter implements Filter
{
    /** log4j category */
    private static Logger log = Logger.getLogger(RegisteredOnlyOpenaireFilter.class);
            ...
            FilterChain chain) throws ServletException, IOException
    {
        ...
        try
        {
            ...
            String token=hrequest.getParameter("tokenEvaluacion");
    ...
    private boolean tokenIsValid(Context context,String token) {
	RevisionToken revisionToken=null;
	if(token!=null){
	    try {
		revisionToken = RevisionToken.find(context, token);
	    } catch (IOException e) {
		log.error("Error en tokenIsValid:",e);
		e.printStackTrace();
	...
	if(revisionToken!=null && revisionToken.getRevisionId()==null){
	    return true;
	    }
	else{
	    return false;
