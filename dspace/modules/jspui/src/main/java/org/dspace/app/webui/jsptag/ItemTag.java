...
public class ItemTag extends TagSupport
{
    ...
    private void itemsRelacionados() throws IOException
        {
            JspWriter out = pageContext.getOut();
            HttpServletRequest request = (HttpServletRequest) pageContext.getRequest();
            DCValue[] items=item.getMetadata("dc.relation.isrelatedtoItem");
            DCValue[] revisiones=item.getMetadata("dc.relation.isrelatedtoRevision");
            DCValue[] juicios=item.getMetadata("dc.relation.isrelatedtoJuicio");
            if(items.length>0){
                out.print("<tr><td class=\"metadataFieldLabel\">");
                out.print(LocaleSupport.getLocalizedMessage(pageContext,"org.dspace.app.webui.jsptag.ItemTag.relation.isrelatedtoItem"));
            ...
            if(revisiones.length>0){
                out.print("<tr><td class=\"metadataFieldLabel\">");
                out.print(LocaleSupport.getLocalizedMessage(pageContext,"org.dspace.app.webui.jsptag.ItemTag.relation.isrelatedtoRevision"));
            ...
            if(juicios.length>0){
                out.print("<tr><td class=\"metadataFieldLabel\">");
                out.print(LocaleSupport.getLocalizedMessage(pageContext,"org.dspace.app.webui.jsptag.ItemTag.relation.isrelatedtoJuicio"));
    
                out.print("</td><td class=\"metadataFieldValue\""+
                		(style.equals("full")?"colspan=\"2\"":"")
                		+">");
                for (int i = 0; i < juicios.length; i++)
                {
                    out.print("<a href=\"");
                    out.print(juicios[i].value);
                    out.print("\">");
                    out.print(juicios[i].value);
                    out.print("</a><br/>");
                }
