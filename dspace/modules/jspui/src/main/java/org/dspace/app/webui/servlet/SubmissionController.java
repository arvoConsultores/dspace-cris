...
public class SubmissionController extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        if(revisionToken!=null && revisionToken.getWorkspaceId()!=null){
            workspaceID=revisionToken.getWorkspaceId();
        }
	...
	if(token!=null){
		...
		if(revisionToken.getTipo().equals(RevisionToken.TIPO_REVISION)){
		    DSpaceObject colection=HandleManager.resolveToObject(context, ConfigurationManager.getProperty("openaire.coleccion.evaluaciones"));
		    request.setAttribute("collectionid",colection.getID());
		}else if(revisionToken.getTipo().equals(RevisionToken.TIPO_JUICIO)){
		    DSpaceObject colection=HandleManager.resolveToObject(context, ConfigurationManager.getProperty("openaire.coleccion.juicios"));
		    request.setAttribute("collectionid",colection.getID());
		}
	    }
	}
