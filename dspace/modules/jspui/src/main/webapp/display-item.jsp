...
<%@page import="es.arvo.openaire.OpenaireUtils"%>
<%
    // Attributes
...
<%
//ARVO: añadimos el boton para solicitar una correccion si es un item, o el de solicitar juicio si es una revision.
		if(OpenaireUtils.isRevision(item.getID())){
			%>  
			<br /> 
			<a class="btn btn-default" href="<%= request.getContextPath() %>/juzgarRequest?handle=<%= handle %>">
		        <fmt:message key="jsp.display-item.solicitarJuicio"/>
		    </a>
		    <% 		        
		}else if(OpenaireUtils.isItem(item.getID())){
			%>
			<br />   
			<a class="btn btn-default" href="<%= request.getContextPath() %>/revisionRequest?handle=<%= handle %>">
		        <fmt:message key="jsp.display-item.solicitarRevision"/>
		    </a>
		    <% 		        
